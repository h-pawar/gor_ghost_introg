...
# Fri 24 Feb 2023 11:53:53 CET
# GHOSTW - revised
# generate model comparison simulations using weighted median posteriors from revised abc parameter inference for ghost gene flow into w_anc - sampling all parameters from priors



#-----------------------------------------------------------------------------------------------------------------------

#Fri 22 Apr 2022 14:14:40 CEST
# generate model comparison simulations using final weighted median posteriors from the final abc parameter inference for ghost gene flow into w_anc


# amending below from 

# Mon 13 Dec 2021 12:40:05 CET
# amending below from test.ghostw.demog.v1.R
# to add filtering step before calculating summary stats of model comparison simulations
  # to remove sites fixed across all gorilla individuals (1/1 across all individuals). - same filtering as previously performed for the empirical data
# filtering relevant for the summary stats
  #  number of population-wise fixed and segregating sites & the number of fixed sites per individual
#-----------------------------------------------------------------------------------------------------------------------
...
# similar to the postabc.sim simulations (but wihtout specifying stepwise seg sites)

# amending the below from # /scratch/devel/hpawar/admix/abc/simul/scripts/ghost.model.v1.R
...
#iter=51 # Q - whether still need to iterate here? b/c not sampling any parameters from priors for the simulations for model comparison?
# MK - You could still run the iterations per job to collect the data, otherwise you run 2500 jobs for each model, with the according number of intermediate and final files...
...
# weighted median posteriors:
# from  /Volumes/"Ultra USB 3.0"/IBE/further.analysis.feb.2020/gorillas/abc/simul_postabc/abc+ghost/generateghostabc.22nov.R
#> summary(ghostabc_22nov_noghostt0)
#Call: 
#abc(target = target1, param = param1, sumstat = sumstat1, tol = 0.005, 
#    method = "neuralnet", numnet = 100)
#Data:
# abc.out$adj.values (179 posterior samples)
#Weights:
# abc.out$weights
#                       w_lowl_t0 w_cros_t0 e_lowl_t0 e_lowl_t2 t_archintrog
#Weighted Median:         83.4695   17.9171   -2.5657   17.9642       2.6866

#                       archaicintrog       t5 admix_w_e_t6 admix_e_w_t6
#Weighted Median:             40.0490   0.5120      70.6732     -20.9973

#                             t9 gor_ghost_anc
#Weighted Median:        58.1226       86.3801
...
# posteriors from generate.ghostw.rev.abc.24feb23.R

# 1) priors for current Ne 
w_lowl_t0=83.9216 # w_lowl_t0 
w_cros_t0=15.8840 # w_cros_t0 
e_lowl_t0=21.0246 # e_lowl_t0 
e_moun_t0=0.4062 # e_moun_t0 
ghost_t0=25
# 2) rapid & severe population decline of E lowland over last 20y (0.2kya) of ~80% - van der valk 2018, plumptr 2016
t1=0.0003947368
e_lowl_t1=0.9848 # e_lowl_t1 
...
t4=0.1990
e_anc_t4=1.6466 # e_anc_t4 
t_archintrog=6.0815
archaicintrog=17.0257
...
# all parameter vals now fixed
# but may still need to specify the random number identifier? # or the identifier could be the array job number?
# inputsets=as.data.frame(t(randtyp[1:iter]))
  # Q - whether to retain this? to not change too many things of the script?
...
all_function<-function(nput) {
  msout <-read.ms.output(paste("/dev/shm/mydata/mc.ghostw.sim_",nput,sep=""))
  ## create empty vector
...
# 14/12/21 - removing sites fixed in all gorilla individuals in the dataset (all gor ids 1/1 relative to human ref) - go to next section **
...
#cofig2<-cbind(c(0,cumsum(config)[-length(config)]),cumsum(config)) # is defined above
...
# ABC + ghost
# change this line - dir/dev/shm/mydata/ghost.abc.sim_",ptype # instead of input13 shoudl just be the rep number
#system(paste("cat /scratch/devel/hpawar/admix/sstar/simul/tetaroAr_corr.txt | /home/devel/hpawar/ms/msdir/ms ",isu," 2500 -seeds ",paste(sample(2000:6000,3),collapse=" ")," -t tbs -r tbs 40000.0 -I 5 54 2 18 24 0 -n 1 ",initial_wl," -n 2 ",initial_wc," -n 3 ",initial_el," -n 4 ",initial_em," -n 5 ",initial_ghost,"  ",simval," > /dev/shm/mydata/mc.ghostw.sim_",input[1],sep=""),intern=F)
  # note here changing input[[13]] to input[[1]] # only the random numbers - fixing all parameters
...
system(paste("cat /scratch/devel/hpawar/admix/sstar/simul/tetaroAr_corr.txt | /home/devel/hpawar/ms/msdir/ms ",isu," 250 -seeds ",paste(sample(2000:6000,3),collapse=" ")," -t tbs -r tbs 40000.0 -I 5 54 2 18 24 0 -n 1 ",initial_wl," -n 2 ",initial_wc," -n 3 ",initial_el," -n 4 ",initial_em," -n 5 ",initial_ghost,"  ",simval," > /dev/shm/mydata/mc.ghostw.sim_",input[1],sep=""),intern=F)
...
save(simuresults,inputsets,file=paste("/scratch/devel/hpawar/admix/abc/simul/test/ghost/revisions_8feb23/modelcomp/ghostw/rev.mc.ghostw_sim",ptype,sep=""))
...
# ABC + ghost
system(paste("rm /dev/shm/mydata/mc.ghostw.sim_",inputsets[[i]][[1]],sep=""),intern=F)
}
