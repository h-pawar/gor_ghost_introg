...
# Tue 12 Apr 2022 16:19:47 CEST
# generate model comparison simulations using final weighted median posteriors from the final abc parameter inference for null demog model

#-----------------------------------------------------------------------------------------------------------------------

# Tue 14 Dec 2021 12:40:37 CET
# remove sites fixed across all gorilla individuals (1/1 across all individuals). - same filtering as previously performed for the empirical data
# before calculating the summary stats
  # same steps as performed in -  /scratch/devel/hpawar/admix/abc/simul/scripts/modelcomp/test.ghoste.demog.v1.2.R 

# Thu  9 Dec 2021 14:22:03 CET
# null demog model + non-interacting ghost
# ie adding 5th pop (set as 25k) & longer timeframe 

#-----------------------------------------------------------------------------------------------------------------------
# building on /scratch/devel/hpawar/admix/abc/simul/scripts/modelcomp/test.null.demog.v1.R
...
# this number of haploid chr to simulate: w_lowl (1), w_cros (2), e_lowl (3), e_moun (4), ghost (5)
  # how many are sampled in the present
...
# posteriors from generateabc.amendtajima.R
...
# 11) posteriors from generateghosteabc.12apr22.R
t9=45.0179
...
all_function<-function(nput) {
  msout <-read.ms.output(paste("/dev/shm/mydata/mc.null.plusghost.sim_",nput,sep=""))
  ## create empty vector
...
#system(paste("cat /scratch/devel/hpawar/admix/sstar/simul/tetaroAr_corr.txt | /home/devel/hpawar/ms/msdir/ms ",isu," 250 -seeds ",paste(sample(2000:6000,3),collapse=" ")," -t tbs -r tbs 40000.0 -I 4 54 2 18 24 -n 1 ",initial_wl," -n 2 ",initial_wc," -n 3 ",initial_el," -n 4 ",initial_em,"  ",simval," > /dev/shm/mydata/mc.null.sim_",input[1],sep=""),intern=F)

#Thu  9 Dec 2021 14:33:38 CET
system(paste("cat /scratch/devel/hpawar/admix/sstar/simul/tetaroAr_corr.txt | /home/devel/hpawar/ms/msdir/ms ",isu," 250 -seeds ",paste(sample(2000:6000,3),collapse=" ")," -t tbs -r tbs 40000.0 -I 5 54 2 18 24 0 -n 1 ",initial_wl," -n 2 ",initial_wc," -n 3 ",initial_el," -n 4 ",initial_em," -n 5 ",initial_ghost,"  ",simval," > /dev/shm/mydata/mc.null.plusghost.sim_",input[1],sep=""),intern=F)
...
#save(simuresults,inputsets,file=paste("/scratch/devel/hpawar/admix/abc/simul/test/modelcomp/nullplusghost/test.mc.nullplusghost_sim",ptype,sep=""))

# Tue 12 Apr 2022 16:44:55 CEST
#mkdir -p /scratch/devel/hpawar/admix/abc/simul/test/modelcomp/final_12apr22/nullplusghost
#mkdir -p /scratch/devel/hpawar/admix/abc/simul/test/modelcomp/final_12apr22/ghoste
#mkdir -p /scratch/devel/hpawar/admix/abc/simul/test/modelcomp/final_12apr22/ghostw

save(simuresults,inputsets,file=paste("/scratch/devel/hpawar/admix/abc/simul/test/modelcomp/final_12apr22/nullplusghost/test.mc.nullplusghost_sim",ptype,sep=""))


# mk new dir for these new null simns
#mkdir -p /scratch/devel/hpawar/admix/abc/simul/test/modelcomp/nullplusghost
...
# ABC + ghost
system(paste("rm /dev/shm/mydata/mc.null.plusghost.sim_",inputsets[[i]][[1]],sep=""),intern=F)
}
