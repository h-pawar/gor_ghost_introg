...
## generate 'high divergence' simulations (& glms) pushing back the divergence time b/n E & W clades of gorilla (to mailund estimate - 429,000)
  # rationale detailed in sstar.notes.23feb.sh & apply.glm.testdemog.R (from testing the demog using mcmanus 2015 divergence time of 261,000y)

#------------------------------------------------------------------------------------------------------------------------

## Thu 18 Feb 2021 12:26:05 CET
# following protocol from troubleshoot.simul.R
# generate new simulations w/out forcing a given number of segregating sites
  # so that the segregating sites instead depend on the demographic model
  # in order to test the demog model - how well it approximates the empirical data
  # & whether it can be reliably used to detect outliers in the empirical data
...
# number of segregating sites - was used for the simulations used to generate the glms 
...
#------------------------------------------------------------------------------------------------------------------------
# specifyign number of segregating sites with -s flag to ms
...
#print(ivec)
# generate the ms simulation 
# 'high divergence' ms simulations
# need to specify seg sites with -s flag & val2
...
## create this object once to store the data
#savelist<-list()
