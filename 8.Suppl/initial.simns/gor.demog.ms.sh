...
-t tbs \  ## scaled mutation rate 4 * N * u * length - pass from tetaroAr_corr.txt (generate for gor)
...
\  # parameters at sampling time - in the present (check scaling ** & whether haploid or diploid ** - assumed diploids?? )
...
\  # 1. E subspecies split # merge eas_moun into eas_lowl ** (adding parameters from xue et al. 2015 psmc into the mcmanus model)
...
\  # # western lowland -> eastern lowland - (30% (0.3) inferred from mcmanus et al 2015 gphocs modelling - v high, but after exploring gphocs.gor.issue.5oct.sh it is consistent within their calculations)
-em 0.4473684 3 1 1200 \  # migration to 3 (eas_lowl) from 1 (wes_lowl) # 0.3*4000 = 1200 # q - how many years or generatiosn shoudl this run for? b/c this is an awfully high rate of migration - would be better to have as a pulse rather than continuous?
...
# 'high divergence' ms simulations - scripts: /scratch/devel/hpawar/admix/sstar/scripts/high.divergence.simul.arr, /scratch/devel/hpawar/admix/sstar/scripts/high.divergence.simul.R
	# update the last 2 events of the simulation
	# use to generate 'high divergence' simulations:
...
# run a set of simulations explicitly stating the E pop size after merging (& see if this makes a diff) - to do **
