# amending from 
#/Volumes/"Ultra USB 3.0"/IBE/further.analysis.feb.2020/gorillas/abc/simul_postabc/plot_pca_sstarskov_27jul22.R # ran interactively
...
#hold_plots[[i]]<-ggarrange(rand_12,rand_34, ncol = 1, nrow = 2)
#}


#SPE="GBG"
#spe="gbg"
#pdf(paste("/scratch/devel/hpawar/admix/overlap.s*.skov/regionsperid/",spe,".1randomreppca.10nov22.pdf",sep="")) 
#hold_plots
#dev.off()
# this is better but need to change the colours, remove the titles & have one legend
...
# need to amend colours *
# bwindi, viruna, gbg, mtt, cr, wl
...
p<-ggplot(data = pc1pc2, aes(x = pc1, y = pc2, color = pop)) +  geom_point(alpha = 0.8) + xlab(xaxis) + ylab(yaxis) + theme_classic() + scale_color_manual(values=c("#2f677d","#4595b5","91C79B","#b8dbbe","#EF7C69","#F2BA5B"))
...
# + scale_color_manual(values=c("#2f677d","#4595b5","91C79B","black","#EF7C69","#F2BA5B"))
...
p<-ggplot(data = pc1pc2, aes(x = pc1, y = pc2, color = pop)) +  geom_point(alpha = 0.8) + xlab(xaxis) + ylab(yaxis) + theme(legend.title=element_blank()) + theme_classic() + scale_color_manual(values=c("#2f677d","#4595b5","91C79B","black","#EF7C69","#F2BA5B"))
...
p<-ggplot(data = pc1pc2, aes(x = pc1, y = pc2, color = pop)) +  geom_point(alpha = 0.8) + xlab(xaxis) + ylab(yaxis) + 
theme_classic()  +  theme(legend.title=element_blank()) + 
	scale_color_manual(values=c("#2f677d","#4595b5","91C79B","#EF7C69","#F2BA5B","#a4b4a7"))
return(p)
}

# colours now fine, just need to adjust size of plots
...
rand_12<-random_plot_pca(random_pca_obj,i,3,4,1,2)
rand_34<-random_plot_pca(random_pca_obj,i,5,6,3,4)
hold_plots[[i]]<-ggarrange(rand_12,rand_34, ncol = 2, nrow = 1,common.legend = TRUE,legend="bottom")
...
# this is fine, now change size of plots/orientation
# & move legend
...
# colours now fine, just need to adjust size of plots
	# adding this to the main figure **
...
#4595b5 # virunga # try this col - to better distinguosh #68abc5
#2f677d # bwindi
#b8dbbe # mt T
